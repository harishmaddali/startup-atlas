...
// The map only shows recent startups; companies.json keeps the full
// historical dataset (including pre-2022 ones) intact regardless.
...
export async function getCompanies(): Promise<Company[]> {
  ...
  return (companiesData as Company[]).filter(
    (c) => c.yearFounded >= MIN_YEAR_FOUNDED
  );
}
